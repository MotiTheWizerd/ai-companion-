...
export class ChatGPTProvider extends BaseProvider {
  ...
  async handleRequest(body, context = {}) {
    ...
    try {
      ...
      console.log(
        "[ChatGPTProvider] Memory fetch enabled:",
        memoryFetchEnabled,
      );

      if (!memoryFetchEnabled) {
        console.log(
          "[ChatGPTProvider] Memory fetch disabled, skipping memory injection",
        );
        return body;
      }
      ...
      if (body?.messages?.[0]?.content?.parts?.[0]) {
        ...
        if (typeof currentText === "string" && currentText.trim().length > 0) {
          try {
            console.log(
              "[ChatGPTProvider] Requesting memory search via background...",
            );
            console.log(
              "[ChatGPTProvider] Project ID: (Waiting for Loader injection)",
            );
            ...
            console.log(
              "[ChatGPTProvider] Search results:",
            ...
            if (
              ...
              console.log(
                "[ChatGPTProvider] Modified prompt:",
            ...
            console.warn(
              "[ChatGPTProvider] Failed to fetch memory context:",
              apiError,
            );
